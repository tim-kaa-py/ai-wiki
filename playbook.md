...
45. **Convert every human code-review comment into a durable repo artifact.** Human review blocking PRs is the merge bottleneck. If a human gave feedback once, encode it so an agent surfaces it forever — via a lint, a structural test, a reviewer-agent prompt, or a persona doc. Order of preference (deterministic → judgment-based): lint → structural test → reviewer agent → persona doc. Push every concern as far down the ladder as it will go. *(Source: Ryan Lopopolo, OpenAI)*

46. **Run reviewer agents per persona on every push.** One agent per durable concern (reliability, front-end architecture, product, scalability, security). Each reads a "what good looks like" doc for its domain and surfaces P2+ issues against the diff. Replaces synchronous human review as the merge gate. Start with one persona, write the doc, wire the agent SDK call in CI, expand. Not every comment blocks merge — bias toward acceptance, not perfection. *(Source: Ryan Lopopolo, OpenAI)*

47. **Write structural tests against the source code itself — a third testing tier.** Distinct from lints (syntax) and unit tests (behavior): assert properties of the source code as text. File length caps (Ryan uses 350), no duplicate zod schemas across the repo, single canonical async helper, package privacy, dependency direction. Add a `tests/structure/` directory; start with a file-length cap; expand from there. These tests give the agent targeted feedback when it drifts. *(Source: Ryan Lopopolo, OpenAI)*

48. **Make every error message a remediation-oriented prompt.** A lint or test failure is a free prompt-injection surface. Bad: "unknown type not allowed." Good: "`unknown` is not allowed in domain code — we parse-don't-validate at the edge, derive a concrete type from the zod schema in `packages/schemas/<entity>.ts`. See `docs/load-bearing-zod.md`." Include the *why* so the agent generalizes. Audit your 10 most-common failure messages and rewrite each as "Don't X because Y. Do Z using helper W." *(Source: Ryan Lopopolo, OpenAI)*

49. **Garbage collection day (weekly ritual).** One full day per week where every engineer's job is to take every bit of slop observed during the week that made PRs hard to merge and categorically eliminate it — turning human-review feedback into repo artifacts (lints, structural tests, reviewer-agent rules, persona-doc updates). This is what makes the reviewer-agent / structural-test infrastructure compound instead of stagnate. *(Source: Ryan Lopopolo, OpenAI)*

50. **Every manual "continue" is a harness failure.** When you click through an agent checkpoint or type "continue" / "yes," stop and ask: what context was the agent missing that it had to stop and ask? Encode that context in a skill, CLAUDE.md, a persona doc, or a structural test so the agent proceeds autonomously next time. Every click is signal that your harness has a gap. *(Source: Ryan Lopopolo, OpenAI)*

51. **Token budget split: a third planning, a third implementation, a third CI.** Writing code is no longer the hard part; getting it accepted is. Reviewer agents, structural-test generation, and PR-commentary agents belong in CI, not in-editor. If your CI token spend is <20% of total, you're under-investing in acceptance. *(Source: Ryan Lopopolo, OpenAI)*

52. **Structure the repo for agent cognition, not headcount — split by domain and stack layer.** A 2-person team with agents needs "10,000-engineer-org" architecture because the agent lacks the tacit domain knowledge a small human team would share verbally. Ryan's team runs a 750-package PNPM workspace. Identify the 3-5 domains your agents keep crossing inappropriately; split them into separate packages with explicit public APIs; lint (or structural-test) package-privacy violations. *(Source: Ryan Lopopolo, OpenAI)*

53. **Make the code uniform across the repo so the agent's predictions transfer.** One way to do bounded concurrency. One ORM. One CI-script style. Appoint a "dictator" on your team for one uniformity decision per month. Write the canonical pattern down. Fire off parallel agents to migrate the rest of the codebase — migrations no longer hang open, because code is free. *(Source: Ryan Lopopolo, OpenAI)*

54. **Outside-in harness: the coding agent is the entry point.** Build the repo so Codex / Claude Code is the entry point, not a guest in a dev shell. Skills — 5-10, not thousands — hide local tooling churn so humans don't track internal changes. Ryan didn't notice for three weeks that his DevTools connection switched from CDP direct to a daemon, because skills abstracted it. *(Source: Ryan Lopopolo, OpenAI)*

55. **Be skeptical of plan mode — unread plans encode unwanted instructions.** A counterpoint to #11 (Boris Cherny's "start every session in Plan mode"). Plans are long; most of the time the engineer won't read every line. Approving a plan is equivalent to approving every instruction in it. Unread approved plans waste tokens on bad rollouts. Default: skip the plan if you won't read it line-by-line; let a well-specified ticket + a good harness suffice. If you must use a plan, ship it as its own PR, review line-by-line, merge, then execute. Reconcile with #11 by asking: will I actually read this plan? If yes, use Plan mode. If no, skip it. *(Source: Ryan Lopopolo, OpenAI)*

56. **Bet on first-party harnesses (Codex, Claude Code) — they're post-trained with the model.** Apply-patch tools and bash-invocation semantics are in the model's post-training loop. Don't reinvent the coding loop from scratch; plug into the SDK where you need custom behavior. This complements #38 (MCP vs CLI) — first-party CLIs have the deepest integration. *(Source: Ryan Lopopolo, OpenAI)*

57. **LLM as fuzzy compiler — code is a disposable build artifact.** Harness context (lints, tests, reviewer prompts) is to an LLM what LLVM static-analysis and optimization passes are to a Rust compiler. Swapping GPT-5 for GPT-6 is like swapping LLVM for Cranelift: different generated instructions, same soundness guarantees, because the constraints on acceptable output live outside the generation backend. Implication: invest in the constraints (harness), not in the generated code — and don't invest in harness pieces that model capability will obsolete (elaborate multi-step orchestrations, hand-tuned prompt templates). *(Source: Ryan Lopopolo, OpenAI)*

58. **QA plans as the forcing function for trusting agent output.** Every user-facing PR attaches a QA plan: features changed, critical user journeys affected, required PR media (screenshots, recordings). A product-reviewer agent asserts the plan was followed. Have your most product-minded engineer write "how to write a good QA plan" once; require every user-facing PR to attach one; wire a reviewer agent to check PR media matches the plan. Lets humans stop shoulder-surfing user-facing changes. *(Source: Ryan Lopopolo, OpenAI)*

59. **Generate skills and prompts with agents, pointed at official cookbooks.** Point your coding agent at the relevant provider's prompting cookbook (OpenAI, Anthropic) and ask it to synthesize a prompt-writing skill. Use that skill every time you need to write a harness prompt. Meta-programming the job, one skill at a time. *(Source: Ryan Lopopolo, OpenAI)*
...
- **Leaving humans synchronously in the PR-review loop at agent velocity.** At 3-5 PRs/engineer/day, human review becomes the merge bottleneck; the same review comments also repeat, so each is a context failure the agent could have avoided. Convert recurring review feedback into lints, structural tests, reviewer agents, and persona docs — let humans review the *review-system*, not every PR. *(Source: Ryan Lopopolo, OpenAI)*
- **Treating every lint/test failure message as a log line instead of a prompt.** An error the agent reads and can't act on is wasted context. Rewrite failures as remediation-oriented prompts with *why* and *how to fix*, pointing at the canonical helper/schema. Every diagnostic is a prompt-injection surface. *(Source: Ryan Lopopolo, OpenAI)*
- **Tolerating recurring manual "continue" clicks.** Every manual checkpoint you click through is signal the harness lacked context for autonomous progress. Encode the missing context once (skill, CLAUDE.md, persona doc, structural test) instead of clicking it forever. *(Source: Ryan Lopopolo, OpenAI)*
