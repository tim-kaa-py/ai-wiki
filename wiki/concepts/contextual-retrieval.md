...
- [Hybrid RAG: Hybrid Search and Graph RAG](./hybrid-rag.md) — a sibling advanced-RAG technique attacking the same silent-retrieval-failure problem from a different angle: dense + sparse/BM25 hybrid search fused via RRF, plus knowledge-graph triplets. (Note the overlap: Contextual Retrieval also stacks BM25 alongside embeddings.)
